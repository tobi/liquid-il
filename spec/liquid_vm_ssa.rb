...
# Optional Shopify/liquid-vm SSA adapter shim for liquid-spec.
#
...
# into /tmp/liquid-vm by default and point this shim at its real SSA adapter.
#
# Manual usage:
#   LIQUID_VM_PATH=/tmp/liquid-vm bundle exec liquid-spec run spec/liquid_vm_ssa.rb
#   LIQUID_VM_SSA_ADAPTER=/tmp/liquid-vm/test/adapters/liquid_vm_ssa.rb bundle exec liquid-spec bench --adapter=spec/liquid_vm_ssa.rb --adapter=spec/liquid_il.rb

require "liquid/spec/cli/adapter_dsl"

repo_path = File.expand_path(ENV.fetch("LIQUID_VM_PATH", "/tmp/liquid-vm"))
adapter_path = ENV["LIQUID_VM_SSA_ADAPTER"]
adapter_path = File.join(repo_path, "test", "adapters", "liquid_vm_ssa.rb") if adapter_path.nil? || adapter_path.empty?
...
if File.file?(adapter_path)
  ...
  LiquidSpec.setup do |_ctx|
    LiquidSpec.skip!(<<~MSG.chomp)
      Shopify/liquid-vm SSA is optional and was not found.
      Run `bundle exec rake liquid_vm:setup` to clone it into /tmp/liquid-vm,
      or set LIQUID_VM_PATH=/path/to/liquid-vm / LIQUID_VM_SSA_ADAPTER=/path/to/adapter.rb.
    MSG
  end
  ...
  LiquidSpec.compile do |_ctx, _source, _compile_options|
    raise "liquid-vm SSA adapter is unavailable"
  end
